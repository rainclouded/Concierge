describe('Integration Test for Task Manager', () => {
  ...
  it('Add New Task', () => {
    ...
    //cy.get('#taskType').select('Room Cleaning');
    //cy.get('option').contains('Room Cleaning').click();
    cy.get('select').select('Room Cleaning');
    ...
    cy.get('#taskType').should('have.value', 'Room Cleaning');
    ...
    cy.contains('td', '202').parent().contains('Delete').click();
  });

  it('Edit Task', () => {
    // Open task details by clicking on the row containing the task
    getTaskRow(101, 'There is a leak in the bathroom sink that needs urgent attention.')
    ...
    cy.get('textarea.w-full.mt-1.p-2.border.rounded.h-32').clear().type('This is an edited description.');
    ...
    cy.get('.fa-edit').parent('button').click();
    cy.get('textarea.w-full.mt-1.p-2.border.rounded.h-32').clear().type(
      'There is a leak in the bathroom sink that needs urgent attention.'
    );
  ...
  it('Delete Task', () => {
    ...
    cy.get('#taskType').select('Maintenance');
    cy.get('textarea[placeholder="Enter task description"]').type('Temporary task for deletion test.');
    cy.contains('button', 'Save').click();
  ...
  it('Assign and Unassign Task', () => {
    ...
    getTaskRow(101, 'There is a leak in the bathroom sink that needs urgent attention.').click();
    ...
    cy.contains('p', 'Assignee:').invoke('text').should('include', 'Unassigned'); // Assignee should revert to "Unassigned"
  ...
  it('Mark Task as Completed', () => {
    ...
    cy.contains('p', 'Assignee:').invoke('text').should('include', 'Unassigned');
